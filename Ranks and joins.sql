...
#Rank films by length (filter out the rows with nulls or zeros in length column). Select only columns title, length and rank in your out
SELECT title,length,rank() over(order by length  asc) as Length_rank
FROM film
...
SELECT title,length,rank() over(partition by rating order by length   asc) as lengh_rank_by_rating
FROM film
